...
namespace mrta {
        ...
        const RepairOptions& options) const
    {
      ...
      /*      const MatrixDouble tauPrev = tauFeas;
            ...
            // Recompute C and EST after B2 — virtual tasks must use current MR timing
            for (int j = 0; j < n; ++j) {
                const auto& P = taskRobots[j];
                if (P.empty()) continue;
                if (isMR[j]) {
                    C[j] = thetaFeas[j] + durTask[j];
                }
                else {
                    const int s = P.front();
                    C[j] = std::isfinite(tauFeas[s][j])
                        ? tauFeas[s][j] + serviceDur(s, j) : 0.0;
                }
            }
